...
```
isaac-sim/    Isaac Sim 6.0.1 + ROS 2 Jazzy, WebRTC-streamed. Everything else
              in this file is about this one; bare filenames below (Dockerfile,
              run.sh, autoload_stage.py, content/, ros2_ws/) are relative to it.
lab0/         The course's Lab 0 environment (handout path C).
```

`lab0/` is **separate and self-contained**: `ubuntu:24.04` + `ros-jazzy-desktop` + MoveIt 2 + Gazebo Harmonic + a pre-built `xarm_ros2` workspace, driven over X11 rather than WebRTC, with its own `lab0/run.sh` and `lab0/README.md`. It shares no layers, no image and no volumes with the Isaac Sim image, and none of the ROS-bridge constraints below apply to it — in particular it *does* set `ENV ROS_DISTRO=jazzy` and source `/opt/ros/jazzy/setup.bash` on every shell, which is only a problem in an image that contains Isaac Sim. Changes to one are not changes to the other, and `lab0/` does not live under `isaac-sim/`.
